...
function EditModal({ visible, onCancel, onOk, editForm, selectedProduct, props }) {
  ...
  const [status, setStatus] = useState(selectedProduct && selectedProduct.category);
  const [Price, setPrice] = useState(selectedProduct && selectedProduct.price);
  const [Product, setProduct] = useState(selectedProduct && selectedProduct.product);
  const [isStatus, setIsStatus] = useState(selectedProduct && selectedProduct.status);
  ...
  return (
    ...
    >
      <Form form={editForm} onFinish={handleSubmit}
        ...
        <Form.Item
          name="product" label="Product Name" rules={[{ required: true, message: 'Enter Product Name' }]}>
